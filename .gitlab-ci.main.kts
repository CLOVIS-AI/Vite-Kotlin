 ...
 */
val siteUrl = "https://vite-kotlin.opensavvy.dev/"
...
gitlabCi {
	...
	fun buildExample(name: String) = job(stage = test, name = "example:$name") {
		jsNode()
		...
		afterScript {
			...
			// Measure bundle size
			shell("echo \"# TYPE production_size gauge\" >>metrics.txt")
			shell($$"""
		...
		environment {
			name("review/${Variable.Commit.Ref.slug}/$name")
			url($$"$URL")
			tier(Development)
		}
	...
	val simpleExample = buildExample("simple")
	val multiProjectExample = buildExample("multi-project")
